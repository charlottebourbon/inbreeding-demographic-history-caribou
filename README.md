...
The repository contains only the analysis scripts. Each script expects its
inputs in a local `data/` directory and writes to a local `output/` (R) or
`results/` (shell) directory, created at runtime. These directories are **not**
part of the repository: the genotype data are on Dryad (see [Data](#data)), and
all outputs are regenerated by running the scripts.

Each script defines `data_dir` / `out_dir` (R) or `$BASE` and related paths at
the top; edit these to match where you place the data. The input files each
script expects are listed in its header comment.
...
## Contact
Questions about the code can be directed to the corresponding author via the paper, or raised as an issue on this repository.
